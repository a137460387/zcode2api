...
>
> 选号循环必须自带等待与人工干预短路，并设总尝试上限（避免长期无号时无限等待）：
> ```js
...
>   if (++pickAttempts > maxPickAttempts) throw new GatewayError({ status: 503, message: `no usable account after ${maxPickAttempts} attempts: ${reason}` })
