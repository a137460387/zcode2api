...
> **实现前必读——T4 账号池的最终契约（经三轮修复后的形态，与下面的 fakePool 示例不同）：**
> `pool.pick(sessionKey)` 返回三种形态，网关必须正确处理：
> - `{ account, waitMs }`：拿到账号。`waitMs` 是"建议节流间隔"（通常 = minIntervalMs）。
> - `{ account: null, waitMs: <正数>, reason }`：暂时无号（账号在节流窗内或冷却中）。
>   **必须等 `waitMs` 后重试**——这是冷启动/连拍场景的正常路径，不是错误。
> - `{ account: null, waitMs: null, warn: 'human action required', reason }`：**需人工干预**
>   （全部停用/需重登/无套餐）。**必须立即失败**，不能按 waitMs 重试（会无限循环）。
>
> `markSuccess` / `markError` / `recordUsage` 都是 **async**，必须 `await`。
> `markError(account, { status, code })` 内部已实现 3012 冷却梯度、401 needsRelogin、
> 1113 noPackage、429 冷却与 strikes 累计（5 次停用）——**网关不需要自己算冷却，只传真实 status/code**。
>
> 选号循环必须自带等待与人工干预短路，并按**累计等待时长**设防
> （**不要用"尝试次数"上限**：节流是池明确背书的合法等待，次数上限会被并发放大——
> 实测按次数设限时 12 并发只有 6 个成功，因为 N 个请求各自计数同一个节流事件）：
> ```js
> const { account, waitMs, warn, reason } = pool.pick(sessionKey)
> if (!account) {
>   if (warn) throw new GatewayError({ status: 503, message: `no usable account: ${reason}`, hint: '需人工处理：登录/启用账号或购买资源包' })
>   const wait = waitMs ?? 0
>   if (wait <= 0 || waitedMs + wait > maxPickWaitMs) throw new GatewayError({ status: 503, code: 3012, message: ..., hint: '账号处于冷却：请稍后重试或增补账号' })
>   waitedMs += wait
...
> waitedMs = 0   // 拿到号后重置
> ```
> `maxPickWaitMs` 默认 15s（超时即失败，避免 HTTP 请求干睡在 30min~24h 的冷却窗里）。
> 注意池对"节流中"（秒级，正常路径）与"冷却中"（分钟~小时级）返回同样的形态，
> 网关靠这个时长阈值区分二者。若客户端/反代超时较短，需相应调大并配置客户端超时。
...
function fakePool(accounts) {
  let i = 0
  return {
    pick: () => (i < accounts.length ? { account: accounts[i++], waitMs: 0 } : { account: null, waitMs: null, warn: 'human action required', reason: 'all accounts cooling down or disabled' }),
    markSuccess: async () => {},
