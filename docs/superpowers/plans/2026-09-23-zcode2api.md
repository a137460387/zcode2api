...
describe('AccountPool error handling', () => {
  it('3012: cooldown 30min, 3rd strike within window → 24h, 5th → disabled', () => {
    const a = add()
    pool.markError(a, { status: 405, code: 3012 })
    expect(store.get(a.id).cooldownUntil).toBe(clock.t + 30 * 60_000)
    clock.t += 60_000
    pool.markError(store.get(a.id), { status: 405, code: 3012 })
    expect(store.get(a.id).cooldownUntil).toBe(clock.t + 30 * 60_000)
    clock.t += 60_000
    pool.markError(store.get(a.id), { status: 405, code: 3012 })
    expect(store.get(a.id).cooldownUntil).toBe(clock.t + 24 * 60 * 60_000)
    store.update(a.id, { cooldownUntil: 0 })
    clock.t += 24 * 60 * 60_000 + 1
    pool.markError(store.get(a.id), { status: 405, code: 3012 })
    pool.markError(store.get(a.id), { status: 405, code: 3012 })
    expect(store.get(a.id).enabled).toBe(false)
  })
  it('401 → needsRelogin; 1113 → noPackage; 429 → 60s cooldown', () => {
    const a = add(), b = add(), c = add()
    pool.markError(a, { status: 401, code: null })
    expect(store.get(a.id).needsRelogin).toBe(true)
    pool.markError(b, { status: 429, code: 1113 })
    expect(store.get(b.id).noPackage).toBe(true)
    pool.markError(c, { status: 429, code: null })
    expect(store.get(c.id).cooldownUntil).toBe(clock.t + 60_000)
  })
  it('markSuccess resets strikes and cooldown', () => {
    const a = add()
    pool.markError(a, { status: 405, code: 3012 })
    pool.markSuccess(store.get(a.id))
  ...
  it('recordUsage accumulates token stats', () => {
    const a = add()
    pool.recordUsage(a.id, { inputTokens: 10, outputTokens: 5 })
    pool.recordUsage(a.id, { inputTokens: 1, outputTokens: 2 })
...
export class AccountPool {
  ...
  markSuccess(account) {
    this.store.update(account.id, {
      cooldownUntil: 0,
      strikes: 0,
      stats: { ...account.stats, lastUsedAt: this.now() },
    })
  }

  markError(account, { status, code }) {
    const acc = this.store.get(account.id) ?? account
    const stats = { ...acc.stats, lastError: { status, code, at: this.now() } }
    if (status === 401) return this.store.update(acc.id, { needsRelogin: true, stats })
    if (code === 1113) return this.store.update(acc.id, { noPackage: true, stats })
    if (code === 3012 || (status === 429 && code !== 1113)) {
      const strikes = (acc.strikes ?? 0) + 1
      ...
      return this.store.update(acc.id, patch)
    }
    return this.store.update(acc.id, { stats })
  }
  ...
  recordUsage(accountId, { inputTokens = 0, outputTokens = 0 } = {}) {
    ...
    const stats = {
      ...acc.stats,
      requests: (acc.stats?.requests ?? 0) + 1,
      inputTokens: (acc.stats?.inputTokens ?? 0) + inputTokens,
      outputTokens: (acc.stats?.outputTokens ?? 0) + outputTokens,
    }
    this.store.update(accountId, { stats })
  }
...
describe('account login & management', () => {
  ...
  it('set/delete/pool-status work', async () => {
    const deps = buildDeps()
    deps.store.save(newAccountFields({ provider: 'bigmodel', type: 'oauth', jwt: 'j', userInfo: { user_id: '9' } }))
    const app = createApp(deps)
  ...
  it('balance refresh stores planCache', async () => {
    const deps = buildDeps()
    deps.store.save(newAccountFields({ provider: 'bigmodel', type: 'oauth', jwt: 'JW', userInfo: { user_id: '5' } }))
...
export function createApp(deps) {
  ...
  app.post('/accounts/login/:provider/poll', panelAuth, async (req, res) => {
    ...
    if (winner.ok) {
      ...
      const account = store.save(newAccountFields({
